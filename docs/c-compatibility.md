...
| `types.pointers` | foundation | Recursive object pointers support address/dereference, array decay, element-scaled +/- arithmetic, pointer difference/comparison, +=/-= offsets, pointer-level qualifiers and qualifier-adding pointee conversions. Zero-parameter function-pointer types are source-spellable; function addresses, indirect calls, null function-pointer conversions and complete compatibility rules remain pending. |
