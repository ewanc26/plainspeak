...
std::string emitBoxedExpr(const Expr *e, const AnalysisResult &analysis) {
    return std::visit([&](auto &&node) -> std::string {
        ...
        if constexpr (std::is_same_v<T, IntLit>) {
            ...
            std::string macro;
            if (type.kind == TypeKind::Floating) {
                if (type.floatingRank == FloatingRank::Float) macro = node.maximum ? "FLT_MAX" : "FLT_MIN";
                else if (type.floatingRank == FloatingRank::LongDouble) macro = node.maximum ? "LDBL_MAX" : "LDBL_MIN";
                else macro = node.maximum ? "DBL_MAX" : "DBL_MIN";
            } else if (type.kind == TypeKind::Boolean) macro = node.maximum ? "1" : "0";
            else if (type.kind == TypeKind::Enumeration) macro = node.maximum ? "INT_MAX" : "INT_MIN";
            else if (type.kind == TypeKind::Integer) {
                if (type.integerRank == IntegerRank::Char) {
                    if (type.charSignedness == CharSignedness::Plain) macro = node.maximum ? "CHAR_MAX" : "CHAR_MIN";
                    else if (type.isUnsigned) macro = node.maximum ? "UCHAR_MAX" : "0";
                    else macro = node.maximum ? "SCHAR_MAX" : "SCHAR_MIN";
                } else if (type.integerRank == IntegerRank::Short) macro = type.isUnsigned ? (node.maximum ? "USHRT_MAX" : "0") : (node.maximum ? "SHRT_MAX" : "SHRT_MIN");
                else if (type.integerRank == IntegerRank::Int) macro = type.isUnsigned ? (node.maximum ? "UINT_MAX" : "0") : (node.maximum ? "INT_MAX" : "INT_MIN");
                else if (type.integerRank == IntegerRank::Long) macro = type.isUnsigned ? (node.maximum ? "ULONG_MAX" : "0") : (node.maximum ? "LONG_MAX" : "LONG_MIN");
                else macro = type.isUnsigned ? (node.maximum ? "ULLONG_MAX" : "0") : (node.maximum ? "LLONG_MAX" : "LLONG_MIN");
            } else macro = "0";
            return "ps_double((double)(" + macro + "))";
...
std::string emitProgram(const std::vector<Stmt *> &program,
    ...
    for (const auto &header : analysis.cHeaders) out << "#include \"" << header << "\"\n";
    out << "\n";
